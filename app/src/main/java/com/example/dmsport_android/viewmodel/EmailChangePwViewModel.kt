...
class EmailChangePwViewModel (
    ...
    fun emailChangePw(new_password: String){
        val emailChangePwRequest = EmailChangePwRequest(new_password)
        viewModelScope.launch(Dispatchers.IO) {
            _emailChangePwResponse.postValue(emailChangePwRepository.emailChangePw(emailChangePwRequest))
        }
    }

    fun findVerifyEmail(email: String){
        val findPwVerifyEmailRequest = FindPwVerifyEmailRequest(email)
        viewModelScope.launch(Dispatchers.IO) {
            _findVerifyEmailResponse.postValue(emailChangePwRepository.findVerifyEmail(findPwVerifyEmailRequest))
        }
    }
